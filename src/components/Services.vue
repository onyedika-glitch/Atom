<template>
  <section class="w-full min-h-screen bg-white px-4 py-16 flex flex-col items-center justify-center text-center">
    <div class="max-w-7xl mx-auto text-center mb-16">
      <h4 class="text-cyan-600 font-semibold text-lg">Services</h4>
      <div class="h-10"></div>
      <h2 class="text-4xl font-bold text-gray-900 mt-2">What We DO</h2>
      <div class="h-10"></div>
      <p class="text-gray-500 max-w-2xl mx-auto mt-4 text-center">
        At ATOM, we deliver cutting-edge, enterprise-grade technology services designed to meet the evolving needs of modern businesses and institutions.
      </p>
    </div>

    <div class="overflow-x-auto">
      <div class="flex gap-6 w-max px-4 animate-slide">
        <div
            v-for="(card, index) in services"
            :key="index"
            class="group relative w-[30rem] h-[20rem] rounded-3xl p-6 overflow-hidden text-white cursor-pointer transition-all duration-500"
            :class="card.bg"
        >
          <!-- Front Content -->
          <div class="absolute inset-0 flex flex-col justify-between p-6 transition-opacity duration-500 group-hover:opacity-0">
            <div class="text-3xl mb-4">
              <img :src="card.icon" alt= "service icon" class="w-12 h-12" />
            </div>
            <div>
              <h3 class="text-2xl font-bold mb-2">{{ card.title }}</h3>
              <p class="text-sm">{{ card.description }}</p>
            </div>
            <p class="text-sm font-bold uppercase">ATOM</p>
          </div>

          <!-- Back Content (fade in on hover) -->
          <div class="absolute inset-0 flex flex-col justify-between p-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10">
            <div>
              <h3 class="text-2xl font-bold mb-4">Services</h3>
              <div v-if="card.backContent" class="flex flex-col gap-2">
                <div v-for="(item, idx) in card.backContent" :key="idx" class="bg-white text-black px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-200">
                  {{ item }}
                </div>
              </div>
            </div>
            <div>
              <button class="bg-black text-white px-6 py-2 rounded-xl mt-6 hover:bg-gray-800">Get Started</button>
              <p class="text-sm font-bold uppercase mt-2">ATOM</p>
            </div>
          </div>

          <!-- Animated Cube SVG -->
          <div class="absolute top-6 right-6 z-0 w-32 h-32 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            <img src="@/assets/animated-cube.png" alt="Cube" class="w-full h-full animated-cube" />
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { gsap } from 'gsap'
import iconAI from '@/assets/service-Icons-5.png'
import iconData from '@/assets/service-Icons-2.png'
import iconSoftware from '@/assets/service-Icons.png'
import iconConsulting from '@/assets/service-Icons-3.png'
import iconAutomation from '@/assets/service-Icons-4.png'


const services = ref([
  {
    title: 'AI & Data Science',
    description: 'We build intelligent solutions that drive efficiency and innovation across industries.',
    bg: 'bg-gradient-to-br from-gray-700 to-gray-900',
    icon: iconAI,
    backContent: [
      'AI-powered chatbots and virtual assistant',
      'Natural Language Processing (NLP) solutions',
      'Custom Large Language Model (LLM) integrations'
    ]
  },
  {
    title: 'Data Engineering & Analytics',
    description: 'We enable data-driven decision-making by transforming raw data into actionable intelligence.',
    bg: 'bg-[#e57368]',
    icon: iconData,
    backContent: [
        'End-to-end data pipeline development (ETL/ELT)kkk',
        'Business intelligence dashboard systems',
        'SEO & performance analytics integration'
    ]
  },
  {
    title: 'Software & Application Development',
    description: 'We engineer robust digital solutions with a focus on reliability, performance, and user experience.',
    bg: 'bg-[#7b6cf6]',
    icon: iconSoftware,
    backContent: [
        'Full-stack web and mobile application development',
        'Custom backend systems and RESTFUL APIs',
        'MVPs and scalable enterprise platforms'
    ]
  },
  {
    title: 'Technical Strategy & Systems Consulting',
    description: 'We work with companies to design, optimize, and scale their digital infrastructure.',
    bg: 'bg-[#23b6c6]',
    icon: iconConsulting,
    backContent: [
        'AI and system architecture design',
        'API strategy and integration planning',
        'Cloud-native infrastructure and deployment pipeline'
    ]
  },
  {
    title: 'Artificial Intelligence Automation',
    description: 'We build intelligent systems that drive efficiency and innovation across industries.',
    bg: 'bg-gradient-to-br from-gray-700 to-gray-900',
    icon: iconAutomation,
    backContent: [
        'AI-powered chatbots and virtual assistants',
        'Natural Language Processing (NLP) solutions',
        'Custom Large Language Model (LLM) integration'
    ]
  }
])

onMounted(() => {
  gsap.to('.group:hover .animated-cube', {
    rotate: 360,
    repeat: -1,
    duration: 10,
    ease: 'linear'
  })
})
</script>

<style scoped>
@keyframes slide {
  0% {
    transform: translateX(0);
  }
  100% {
    transform: translateX(-25%);
  }
}
.animate-slide {
  animation: slide 30s linear infinite;
}
</style>
